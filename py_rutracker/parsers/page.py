...
class ParsingPage:
    ...
    def extract_search_id(html: str) -> Optional[str]:
        ...
        pg_base_url_pattern = (
            r"PG_BASE_URL:\s*['\"]tracker\.php\?search_id=([^'\"]+)['\"]"
        )
        ...
        for pattern in js_search_id_patterns:
            ...
            if match:
                ...
                if len(search_id) > 5 and " " not in search_id:
                    logger.debug(
                        f"search_id найден в JavaScript переменной: {search_id}"
                    )
                    return search_id
        ...
        hidden_inputs = soup.find_all(
            "input", type="hidden", attrs={"name": "search_id"}
        )
                ...
                for attr_name, attr_value in element.attrs.items():
                    if attr_name.startswith("data-") or "search_id=" in str(attr_value):
                        if isinstance(attr_value, str) and "search_id=" in attr_value:
                            search_id_match = re.search(
                                r"search_id=([^&\s\"']+)", attr_value
                            )
                            if search_id_match:
                                search_id = search_id_match.group(1)
                                logger.debug(
                                    f"search_id найден в атрибуте {attr_name}: {search_id}"
                                )
                                return search_id
